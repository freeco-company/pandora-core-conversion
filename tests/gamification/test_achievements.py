...
async def test_award_inserts_user_achievement_and_credits_tier_xp(client, db_session):
    ...
    assert body["code"] == "meal.streak_7"
...
async def test_award_writes_ledger_entry_and_progression(client, db_session) -> None:
    ...
    resp = await client.post(
        ...
        json={
            ...
            "code": "meal.streak_30",  # gold tier = 300 XP
            "source_app": "meal",
    ...
    assert len(grants) == 1
    assert grants[0].code == "meal.streak_30"
    ...
    assert len(ledger) == 1
    assert ledger[0].event_kind == "achievement.meal.streak_30"
...
async def test_award_enqueues_achievement_awarded_outbox_event(
    ...
    await client.post(
        ...
        json={
            ...
            "code": "meal.first_meal",  # bronze 30 XP — no level-up at this XP
            "source_app": "meal",
            "idempotency_key": f"ach-{user}-first_meal",
            "occurred_at": _now(),
        },
    ...
    assert achievement_events[0].payload["code"] == "meal.first_meal"
...
async def test_seed_updates_existing_when_catalog_changes(client, db_session, monkeypatch):
    ...
    original = catalog.ACHIEVEMENT_CATALOG["meal.first_meal"]
    monkeypatch.setitem(
    ...
    row = (
        await db_session.execute(
            select(Achievement).where(Achievement.code == "meal.first_meal")
        )
    ).scalar_one()
